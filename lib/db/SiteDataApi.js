...
async function convertNotionToSiteData(
  ...
  // 默认按发布时间倒序（最新在前）；仅当显式配置为 notion 时保留 Notion 视图顺序
  const sortBy = String(
    NOTION_CONFIG?.POSTS_SORT_BY ||
      BLOG.POSTS_SORT_BY ||
      'date'
  ).toLowerCase()
  if (sortBy !== 'notion') {
    allPages.sort(
      (a, b) => Number(b?.publishDate || 0) - Number(a?.publishDate || 0)
    )
  }
...
function handleDataBeforeReturn(db) {
  ...
  // Movie 可使用独立标签，不按文章 tagOptions 过滤，避免观影分类被清掉
  db.allMovies = Array.isArray(db.allMovies) ? db.allMovies : []
  db.latestPosts = cleanPostSummaries(cleanPages(db.latestPosts, db.tagOptions))

  // 定时发布：检查发布时间窗口，超出范围的隐藏
  // 仅对 Post/Page 生效，Event / Member / Movie 不受此限制
  ...
  if (POST_SCHEDULE_PUBLISH) {
    db.allPages?.forEach(p => {
      if (p.type === 'Event' || p.type === 'Member' || p.type === 'Movie') return
...
/**
 * 获取所有已发布的观影记录
 * 从 allPages 中筛选 type=Movie && status=Published 的条目
 */
export function getAllMovies({ allPages }) {
  const published = sortTypedPagesByPublishDate(
    getPublishedTypedPages({ allPages, type: 'Movie' })
  )

  return published.map(m => {
    const ratingRaw = Array.isArray(m.rating) ? m.rating[0] : m.rating
    const kind = Array.isArray(m.kind) ? m.kind[0] || '' : m.kind || ''
    const director = Array.isArray(m.director)
      ? m.director[0] || ''
      : m.director || ''
    const actors = Array.isArray(m.actors)
      ? m.actors.filter(Boolean)
      : m.actors
        ? String(m.actors)
            .split(/[,，、]/)
            .map(s => s.trim())
            .filter(Boolean)
        : []
    const people = [...new Set([director, ...actors].filter(Boolean))]
    return {
      id: m.id || '',
      title: m.title || '',
      type: 'Movie',
      ...
      actors,
      people,
      rating: ratingRaw != null && ratingRaw !== '' ? String(ratingRaw) : '',
      pageIcon: m.pageIcon || '',
      pageCover: m.pageCover || '',
      pageCoverThumbnail: m.pageCoverThumbnail || '',
      publishDate: m.publishDate ?? null,
      publishDay: m.publishDay || '',
      date: m.date || null
    }
  })
}
