...
export default async function getPageProperties(
  ...
  // 观影：导演（单个）+ 主演（列表）；支持 text / select / multi_select
  const directorNames = normalizePersonNames(properties.director)
  properties.director = directorNames[0] || ''
  properties.actors = normalizePersonNames(properties.actors)
...
/**
 * 将 Notion 人物字段归一为去重姓名数组
 * 兼容 text、select、multi_select，以及中英文逗号/顿号分隔
 */
...
export function adjustPageProperties(properties, NOTION_CONFIG) {
  ...
  if (properties.type === 'Post') {
    ...
    // 统一为 movie/{slug}，对应路由 /movie/[slug]
    let raw = String(properties.slug || properties.id || '')
      .replace(/^\/+/, '')
      .trim()
    if (raw.toLowerCase().startsWith('movie/')) {
      raw = raw.slice('movie/'.length)
    }
    properties.slug = `movie/${raw}`
    properties.href = `/movie/${raw}`
